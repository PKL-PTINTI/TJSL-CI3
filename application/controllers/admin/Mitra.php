...
class Mitra extends CI_Controller {
				...
				if(($cicilanOpex->id_akun == '101060201' OR $cicilanOpex->id_akun == '101060202' OR $cicilanOpex->id_akun == '101060203' OR $cicilanOpex->id_akun == '101060204' OR $cicilanOpex->id_akun == '101060205' OR $cicilanOpex->id_akun == '101060206' OR $cicilanOpex->id_akun == '101060207' OR $cicilanOpex->id_akun == '101060208') AND $cicilanOpex->tampil = '1'){
					...
					if($this->data['mitra']->rekondisi == '1'){
						...
						if($angsuranjumlahpaskaRekondisi < $jumlahuangseharusnya){
							$selisihcicilan= ($jumlahuangseharusnya-$angsuranjumlahpaskaRekondisi);
	...
	public function createExcel($no_kontrak) {
		...
		$sheet->setCellValue('C5', $no_kontrak);
		$sheet->setCellValue('C6', $this->data['mitra']->nama_peminjam);
		$sheet->setCellValue('C7', $this->data['mitra']->tglkontrak);
		$sheet->setCellValue('C8', $this->data['mitra']->cicilanperbln);
		$sheet->setCellValue('C9', $this->data['mitra']->pinjjumlah);

		$sheet->setCellValue('F5', $this->data['mitra']->kolektibilitas);
		$sheet->setCellValue('F6', $this->data['mitra']->pinjjasa);
		$sheet->setCellValue('F7', $this->data['mitra']->tgljatuhtempo);
		$sheet->setCellValue('F8', $this->data['mitra']->pinjpokok);
		$sheet->setCellValue('F9', $this->data['mitra']->pinjjasa);

		foreach ($dataOpex as $key => $value) {
			$sheet->setCellValue('A'.($key+12), $value['no']-1);
			$sheet->setCellValue('B'.($key+12), $value['tanggal']);
			$sheet->setCellValue('C'.($key+12), number_format($value['cicil_pokok']));
			$sheet->setCellValue('D'.($key+12), number_format($value['cicil_jasa']));
			$sheet->setCellValue('E'.($key+12), number_format($value['jumlah']));
			$sheet->setCellValue('F'.($key+12), number_format($value['sisaPinjamanJumlah']));
		}
