...
class Mitra extends CI_Controller {
	...
	public function statussektor($status, $sektor){
		$data = [
			'title' => 'Mitra ' . ucfirst($status) . ' ' . ucfirst($sektor),
			'header' => 'Data Mitra ' . ucfirst($status) . ' ' . ucfirst($sektor),
			'mitra' => $this->mitra_model->getMitraStatusSektor($status, $sektor),
		];
	...
	public function rincianCicilan($no_kontrak){
		...
		foreach($this->mitra_model->getOpexCicilan() as $cicilanOpex){
			...
			if($no_pk == 'PK'.$no_kontrak){
				...
				if($cicilanOpex->id_akun == '101060201' || $cicilanOpex->id_akun == '101060202' || $cicilanOpex->id_akun == '101060203' || $cicilanOpex->id_akun == '101060204' || $cicilanOpex->id_akun == '101060205' || $cicilanOpex->id_akun == '101060206' || $cicilanOpex->id_akun == '101060207' || $cicilanOpex->id_akun == '101060208'){
					...
					if($data['mitra']->rekondisi == '1'){
						$tanggalkontrak = $data['mitra']->tgl_rekondisi;
						...
						if($angsuranjumlahpaskaRekondisi< $jumlahuangseharusnya){
							...
							if($selisihbrpkalicicilan > 0 AND $selisihbrpkalicicilan <= 6){
								...
								{
									$forcekolek='kurang lancar';
								}

								if($kolektibilitasawal=='kurang lancar')
								{
									$forcekolek='diragukan';
								}
							...
							$kolektibilitas = $forcekolek;
						}

						$this->mitra_model->updateKolektibilitas($nokontrak, $kolektibilitas);
					} else {
						$tgl_pertama = strtotime(date('Y-m-01', strtotime(date('Y-m-d'))));
	...
	public function tambahCicilan(){
		...
		$cicilan_jasa = $this->input->post('cicilan_jasa');
